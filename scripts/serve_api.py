"""OutcomeRail read-only WSGI API'sini yalnız local loopback üzerinde çalıştırır."""
...
def _loopback_host(value: str) -> str:
    try:
        ...
        raise argparse.ArgumentTypeError("host bir loopback IP adresi olmalı") from exc
    if not isinstance(address, ipaddress.IPv4Address) or not address.is_loopback:
        raise argparse.ArgumentTypeError("host yalnız IPv4 loopback adresi olabilir")
    return value


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="OutcomeRail read-only Analysis API")
    parser.add_argument("--host", default="127.0.0.1", type=_loopback_host, help="Yalnız loopback IP adresi")
